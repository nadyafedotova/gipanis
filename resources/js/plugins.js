...
function filterRatingTable(type)
{
    $( 'table tbody tr' ).each(function(e) {
        var rating = parseFloat($(this).find('td:eq(4)').text());
        ...
        switch (type)
        {
            case 0:
                ...
                break;
            case 1:
                if(rating == 0) $tr.removeClass('hide'); else $tr.addClass('hide');
                break;
            case 2:
                if(rating >= 3.8) $tr.removeClass('hide'); else $tr.addClass('hide');
                break;
            case 3:
                if(rating< 3.8 && rating >= 3.4) $tr.removeClass('hide'); else $tr.addClass('hide');
                break;
            case 4:
                if(rating < 3.4 && rating != 0) $tr.removeClass('hide'); else $tr.addClass('hide');
                break;
        }
    });
}
